...
同时保留 Hysteria2、用户、套餐、订单、订阅、流量限制、到期时间和一键迁移部署这些基础能力。v2.2.0 在 v2.1.2 基础上升级商业运营仪表盘，补齐流量可视化、生产空数据体验、流量样本保留期、图表失败降级、发布部署保护、浏览器图标资源、静态资源缓存头、固定默认运行镜像、发布包边界保护、流量采集可靠性、找回密码入口收敛、可信反代客户端 IP 解析、前端启动恢复页和备份恢复原子校验；套餐仍按 USD 计价，管理员只需要配置收款地址，系统即可生成二维码并通过链上查询自动验账，适合小规模自托管、单机多出口实验，以及需要快速复刻部署环境的个人开发者。
...
| 管理端运营 | v2.2.0 包含商业运营仪表盘、连续流量趋势图、用户排行、节点排行和套餐分布，并强化部署前检查 |
