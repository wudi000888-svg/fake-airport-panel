...
function section(title, hint, orders, payments, action) {
  return `
    ...
    <div class="card-list">${orders.map((order) => orderCard(order, payments)).join("") || empty("暂无订单", "open-plans", "购买套餐")}</div>
  `;
}
...
export function renderUserOrders(data = {}) {
  ...
  return `
    <section class="screen stack">
      ...
      ${section("待处理", "待付款、确认中和未完成订单", buckets.active, payments, "orders-pending")}
      ${section("需要补 TXID", "链上记录有歧义时显示在这里", buckets.ambiguous, payments, "orders-ambiguous")}
      ${section("历史订单", "已完成、已取消和过期订单", buckets.history, payments, "orders-history")}
    </section>
  `;
}
